    ...
    protected override void Move()
    {
        if (pv.IsMine)
        {
            ...
            if (Vector3.Distance(this.transform.position, targetPos) <= 1f)
            {
                ...
                hitInfo.transform.GetComponent<Status>().Damage(status.attackDamage, Anivia.transform.name, Status.AttackType.AD, status.ADperPenetration, status.ADpenetration);
