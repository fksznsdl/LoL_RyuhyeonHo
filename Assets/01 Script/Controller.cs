...
public abstract class Controller : MonoBehaviour
{
            ...
            else if(isAttack)
            {
                if (Input.GetMouseButtonDown(1)) {
                    if (Physics.Raycast(playerCamera.ScreenPointToRay(Input.mousePosition), out info)){
                        if (info.transform.name != HitInfo.transform.name)
                        {
                            StopAllCoroutines();
                            isAttack = false;
    ...
    private void RightClick()
    {
        ...
        if (!isMiniMap) {
            ...
            if (Physics.Raycast(playerCamera.ScreenPointToRay(Input.mousePosition), out info))
            {
                ...
                if (info.transform.tag == "Champion" || info.transform.tag == "Minion" || info.transform.tag == "Building" || info.transform.tag == "Monster")
                {
                    if (Vector3.Distance(info.point, chTransform.position) <= status.attackRange)
                    {
                        if (status.team != info.transform.GetComponent<Status>().team)
                        {
                            HitInfo = info;
                            TryAttack(HitInfo);
                        }
                ...
                else if (info.transform.tag == "shop")
                {
                    info.transform.GetComponent<Shop>().ShowShop(status, chTransform);
                }
                else
                {
                    SetDestination(info.point);
    ...
    private void TryAttack(RaycastHit _hitInfo)
    {
        ...
        if (_hitInfo.transform.GetComponent<Status>().isDead)
        {
            return;
        }
